...
def flatten_config(config, prefix=''):
    """
    Flattens nested dictionaries to flat dictionary.

    All keys get concatenated and separated with the dot.
    """
    flattened = {}
...
class Logger:
    ...
    def __init__(self, output_dir=None, output_fname='progress.txt',
                 exp_name=None, neptune_experiment=None):
        """
        Initialize a Logger.
        ...
        Args:
            ...
            neptune_experiment (neptune.experiments.Experiment):
                Neptune experiment to log to. If None, then logging to Neptune
                is disabled.
        """
        if proc_id() == 0:
    ...
    def save_config(self, config):
        ...
        if proc_id() == 0:
            ...
            with open(osp.join(self.output_dir, "config.json"), 'w') as out:
                out.write(output)
            # Log to Neptune if initialized.
            if self.neptune_experiment:
                print(colorize('Saving config to Neptune...\n', color='cyan'))
                config_flattened = flatten_config(config_json)
                for key, val in config_flattened.items():
                    self.neptune_experiment.set_property(key, val)
        ...
        if proc_id() == 0:
            ...
            for key in self.log_headers:
                ...
                if self.neptune_experiment:
                    ...
                    self.neptune_experiment.log_metric(key, step, val)
...
class DummyLogger:
    ...
    def log_tabular(self, *args, **kwargs):
        pass
